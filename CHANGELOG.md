...
- Publish distinct Retail and WoW Forever packages with release metadata kept in sync.

## [1.5.17] - 2026-09-18

### Added

- Provisional WoW Forever support through the client-safe Camelot package.
...
- Player movement diagnostics now retain the readable loss-of-control category,
  spell ID and display text observed at the moment an effect is applied. This
  makes persistent hazards such as Gulf of Memory's Webs diagnosable after the
  transient Blizzard API entry has expired, without adding a location rule.
...
- Warlock Imp support through **Singe Magic**, a friendly Magic dispel that
  removes one effect. Salve enables it only while the Imp's Command Demon
  action is active and refreshes when the Warlock changes demon.
