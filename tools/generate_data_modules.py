...
def load_sources() -> tuple[dict, dict[str, list[dict]], dict[str, dict]]:
...
def render_data(module: dict, records: list[dict]) -> str:
...
def main() -> int:
    ...
    try:
        config, records, modules = load_sources()
    ...
    for folder, module in modules.items():
        outputs = {
            ...
            ROOT / folder / "Data.lua": render_data(module, records[folder]),
        }
    ...
    if args.check and changed:
        ...
        return 1
    if not args.check:
        print(f"generated {len(modules)} data modules from {sum(map(len, records.values()))} records")
    return 0
