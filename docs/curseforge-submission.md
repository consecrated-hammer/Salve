# CurseForge submission

Public page: https://www.curseforge.com/wow/addons/salve

## Summary

A compact clickable dispel panel built for WoW: Midnight's private aura rules.

## Description

Use `README.md` from **Why Salve exists** through **Limitations**. It already
contains the player-facing explanation, setup, commands and honest sound-data
limits without implementation detail.

## Manual screenshot upload order

- [ ] `screenshots/panel-unit-names.png` — Compact group panel with names.
- [ ] `screenshots/panel-dispellable-cells.png` — Cells lit by dispel type.
- [ ] `screenshots/options-preview-and-layout.png` — Pinned live preview and layout.
- [ ] `screenshots/options-cell-size-and-alignment.png` — Cell sizing and text alignment.
- [ ] `screenshots/options-visibility.png` — Visibility, opacity and handle controls.
- [ ] `screenshots/options-dispels-and-alerts.png` — Detected dispel, clicks and alerts.
- [ ] `screenshots/options-commands.png` — Complete slash-command reference.

Available extras:

- `screenshots/panel-clear-cells.png`
- `screenshots/options-cell-text-and-appearance.png`

Retake after loading 1.0.0 before uploading; these captures show old build text:

- [ ] `screenshots/options-troubleshooting.png`
- [ ] `screenshots/options-about.png`

## Manual project fields

- [ ] Upload `Textures/Salve-curseforge-400.png` as the project icon.
- [ ] Set game version to Retail / 12.1.0.
- [ ] Confirm category: Raid Frames or Buffs & Debuffs.
- [ ] Confirm source and issue links point to GitHub.
- [ ] Confirm licence: GPL-3.0.
